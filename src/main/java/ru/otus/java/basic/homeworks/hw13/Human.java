...
public class Human implements Transport {
    ...
    private String name;
    private Transport currientTransport;


    private void changeTranspor(Transport transport) {
        this.currientTransport = transport;
    }

    public boolean go(int distance, TerritoryType territoryType) {
        if (currientTransport!=null)
            return currientTransport.carry(distance, territoryType);
        return false;
    }

private void leaveTransport(){
        this.currientTransport=this;
}

    @Override
    public boolean carry(int distance, TerritoryType territoryType) {
        return false;
    }
